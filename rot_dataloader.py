...
class GenericDataset(data.Dataset):
    def __init__(self, dataset_name, split,
        ...
        if self.dataset_name=='miniimagenet':
            ...
            split_data_dir = os.path.join(_IMAGENET_DATASET_DIR, self.split)
            if not os.path.exists(split_data_dir):
                raise ValueError(f"数据集路径不存在: {split_data_dir}")
            print(f"加载数据集: {split_data_dir}")
...
def flip_img(img, flip_type):
    if flip_type == 'horizontal':
        ...
        raise ValueError('flip_type should be "horizontal" or "vertical"')
        ...
        if self.unsupervised:
            ...
            def _load_function(idx):
                ...
                rotated_imgs = [
                    ...
                    self.transform(rotate_img(img0, 180)),
                    self.transform(rotate_img(img0, 270)),
                    self.transform(flip_img(img0, 'horizontal')),  # 4: 水平翻转
                    self.transform(flip_img(img0, 'vertical'))  # 5: 垂直翻转
                ]
...
if __name__ == '__main__':
    ...
    for i in range(data.size(0)):
        plt.subplot(int(data.size(0)/6),6,i+1)
