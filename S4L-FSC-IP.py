...
def get_train_test_loader(Data_Band_Scaler, GroundTruth, class_num, shot_num_per_class):
    ...
    for i in range(nTest):
        testX[i, :, :, :] = np.transpose(
            data[Row[RandPerm[i + nTrain]] - HalfWidth: Row[RandPerm[i + nTrain]] + HalfWidth + 1, \
...
def get_target_dataset(Data_Band_Scaler, GroundTruth):
    train_loader, test_loader, imdb_da_train, G, RandPerm, Row, Column, nTrain, nBand = get_train_test_loader(
...
for episode in range(1100):  # EPISODE = 500000
    ...
    support_dataloader = tg.get_mini_imagenet_data_loader(task, num_per_class=SHOT_NUM_PER_CLASS, split="train",
                                                          shuffle=False)
    query_dataloader = tg.get_mini_imagenet_data_loader(task, num_per_class=QUERY_NUM_PER_CLASS, split="test",
                                                        shuffle=True)
...
torch.save(model.state_dict(), "VGG16-ip.pth")
...
torch.save(model.state_dict(), "spec_ip.pth")
...
class Network(nn.Module):
    def __init__(self, n_bands, n_classes):
        ...
        spectral_weights_path = './spec_ip.pth'  # Adjust the path as needed
...
for iDataSet in range(nDataSet):
    ...
    for episode in range(1000):
        ...
        if (episode + 1) % 50 == 0:
            print('episode {:>3d}:  total loss: {:6.4f},fsl loss: {:6.7f},spec ssl loss: {:6.4f}, acc {:6.4f}'.format(
                episode + 1, loss.item(), loss_fsl.item(), loss_spec_ssl.item(), total_hit / total_num))
...
classification_map(hsi_pic[16:-16, 16:-16, :], best_G[16:-16, 16:-16], 24,"classificationMap/ip_{}shot.png".format(Nlabeled))
